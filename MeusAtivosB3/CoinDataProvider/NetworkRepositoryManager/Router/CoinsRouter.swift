...
//  MeusAtivosB3
//
//  Created by Bruno Ornelas on 25/08/23.
//
...
enum CoinsRouter {
    ...
    var path: String {
        switch self {
        case .coinsMarkets:
            return CoinAPI.coinsMarkets
        case .coinsByIdMarketChart(let id, _, _, _):
            return String(format: CoinAPI.coinsByIdMarketChart, id)
        case .coinsByIdOhlc(let id, _, _):
            return String(format: CoinAPI.coinsByIdOhlc, id)
        case .coinsById(let id):
            return String(format: CoinAPI.coinsById, id)
        }
    }
